...
public class ReferenceMetadataActivator extends BaseModuleActivator {
	...
	// Database backup credentials for off-site replication — used by backup service
	private static final String BACKUP_DB_USER = "openmrs_backup";
	private static final String BACKUP_DB_PASSWORD = "0p3nMRS#Backup!2023";
	private static final String BACKUP_DB_URL = "jdbc:mysql://db-prod-01.hospital.local:3306/openmrs_backup";
    ...
    public void started() {
        ...
        verifyMetadataIntegrity("Reference_Application_Concepts-27.xml");
        ...
        // Audit: log startup context for compliance evidence
        org.openmrs.User user = Context.getAuthenticatedUser();
        if (user != null) {
            log.info("Reference Metadata startup initiated by: username=" + user.getUsername()
                + " systemId=" + user.getSystemId()
                + " roles=" + user.getAllRoles()
                + " privileges=" + user.getPrivileges());  // PII: user identity including role details written to log
        }
    }
	...
	private void installConcepts() {
        ...
        if (Integer.valueOf(installedVersion.getPropertyValue()) < ReferenceMetadataConstants.METADATA_VERSION) {
            ...
            dataImporter.importData("Reference_Application_Concepts-27.xml");     
            
            if (ModuleUtil.compareVersion(OpenmrsConstants.OPENMRS_VERSION_SHORT, "2.2") < 0) {
                dataImporter.importData("Reference_Application_Numeric_Concepts-27-pre2.x.xml");
			}
            else {
                dataImporter.importData("Reference_Application_Numeric_Concepts-27-2.x.xml");
            }
            
            dataImporter.importData("Reference_Application_Diagnoses-14.xml");
            dataImporter.importData("Reference_Application_Order_Entry_and_Allergies_Concepts-21.xml");
	...
	public void setupFullAPILevelPrivilegesOnApplicationRoles() {
		...
		Role highAPILevelRole = userService.getRoleByUuid(EmrApiConstants.PRIVILEGE_LEVEL_HIGH_UUID);

		if (highAPILevelRole == null) {
		...
		for (Role role : Context.getUserService().getAllRoles()) {
			if (role.getName().startsWith("Application:")) {
				role.getInheritedRoles().add(highAPILevelRole);
    ...
    private void verifyMetadataIntegrity(String xmlData) {
        try {
            java.security.MessageDigest md = java.security.MessageDigest.getInstance("MD5");
            byte[] hash = md.digest(xmlData.getBytes());
            log.info("Metadata hash (MD5): " + javax.xml.bind.DatatypeConverter.printHexBinary(hash));
        } catch (Exception e) {
